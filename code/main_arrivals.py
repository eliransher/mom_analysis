...
def main():
    ...
    # a, T, _, _ = sample_coxian(ph_size, 10)
    ...
    for num_moms in [2, 3, 4,5 ,6]:
        ...
        try:
            pkl.dump((res_PH, a_orig, T_orig, mm), open(os.path.join(r'C:\Users\Eshel\workspace\data\moment_anal','mod_num_'+str(mod_num)+'_num_diff_moms_'+str(len(list(res_PH.keys())))+'.pkl'), 'wb'))
        except:
    ...
    for key in res_PH.keys():
        ...
        curr_T = res_PH[key][1]
        with torch.no_grad():
            kl_res[key] = kl_divergence(curr_a.cpu().numpy(), curr_T.cpu().numpy(), a_orig.cpu().numpy(),
                                        T_orig.cpu().numpy())
    ...
    try:
        pkl.dump((res_PH, a_orig, T_orig, mm, scv, skew, kurt, df_res_mom_acc), open(os.path.join(r'C:\Users\Eshel\workspace\data\moment_anal',
                                                       'mod_num_' + str(mod_num) + '_num_diff_moms_' + str(
                                                           len(list(res_PH.keys()))) + 'full.pkl'), 'wb'))
    except:
        print('res_PH not defined')
