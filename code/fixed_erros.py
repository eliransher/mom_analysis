...
def main(orig_dist_type):
    ...
    min_loss_dict = {2: 1e-7, 3: 1e-7, 4: 1e-13, 5: 1e-13, 6: 1e-13, 7: 1e-13}
    min_loss_dict_errors = {2: 0.01, 3: 0.05, 4: 0.5, 5: 0.5, 6: 0.5, 7: 0.5}
    now = time.time()
    ...
    print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
    error_moms = {3: 1, 4: 5, 5: 10, 6: 15, 7: 20}
    res_PH = {}
    for num_moms in [2, 3]: # , 4, 5, 6, 7
        fit_moms = mm[:num_moms]
        error_fit_moms = []
        for mom in range(num_moms + 1, 8):
            error_fit_moms.append(mm[mom - 1] - mm[mom - 1] * error_moms[mom] / 100)
        cat_dim1 = torch.cat((fit_moms, torch.tensor(error_fit_moms)))
